...
```text
TRIAL,<trial_index>,<baseline_cpi>,<randomized_cpi>,<start_scale>,<knob_min>,<knob_max>,<effective_min>,<effective_max>,<mouse_input_cpi>,<command_id>
```

Example:

```text
TRIAL,1,800,1200,0.18750000,0.25000000,4.00000000,100,6400,6400,42
```

`command_id` is generated by Python. A command is successful only after Python
receives an `ACK` with the same `cmd` and `command_id` and `status=OK`.

On `TRIAL`, Teensy flushes pending telemetry under the previous trial/gain,
resets both knob counters, preserves fractional movement remainders, and starts
at:

```text
randomized_gain = randomized_cpi / baseline_cpi
start_scale = randomized_cpi / mouse_input_cpi
...
effective_scale = effective_cpi / mouse_input_cpi
baseline_effective_scale = effective_cpi / baseline_cpi
```
...
| `mouse_input_cpi` | `u16` | CPI configured on the physical mouse |
